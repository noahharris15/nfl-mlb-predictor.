...
SIM_TRIALS = 10_000
SEASON = "2025-26"   # ✅ YOUR REQUEST: SEASON LOCKED TO 2025–26 ONLY

# ---------------- DEFENSE RATINGS ----------------
# Keys MUST match Odds API team names (home_team / away_team)
DEF_RATINGS = {
    "Oklahoma City Thunder": 1.017,
    "Detroit Pistons": 1.077,
    "Miami Heat": 1.084,
    "Houston Rockets": 1.088,
    "Orlando Magic": 1.090,
    "Golden State Warriors": 1.093,
    "Minnesota Timberwolves": 1.097,
    "Toronto Raptors": 1.097,
    "Dallas Mavericks": 1.098,
    "Memphis Grizzlies": 1.100,
    "Philadelphia 76ers": 1.101,
    "New York Knicks": 1.103,
    "Cleveland Cavaliers": 1.103,
    "Phoenix Suns": 1.111,
    "San Antonio Spurs": 1.113,
    "Atlanta Hawks": 1.114,
    "Boston Celtics": 1.116,
    "Denver Nuggets": 1.130,
    "Indiana Pacers": 1.131,
    "Portland Trail Blazers": 1.139,
    "Los Angeles Lakers": 1.139,
    "Charlotte Hornets": 1.141,
    "Milwaukee Bucks": 1.143,
    "Brooklyn Nets": 1.145,
    "Chicago Bulls": 1.145,
    "Los Angeles Clippers": 1.157,
    "Sacramento Kings": 1.167,
    "Utah Jazz": 1.175,
    "New Orleans Pelicans": 1.176,
    "Washington Wizards": 1.201,
}
...
def get_team_name_cached(pid: int):
    try:
        df = commonplayerinfo.CommonPlayerInfo(pid).get_data_frames()[0]
        t = df["TEAM_NAME"].iloc[0]
...
@st.cache_data(show_spinner=False)
def cached_gamelog(pid: int, season: str):
    """Cached version → FAST."""
    return fetch_gamelog(pid, season)

# ------------------- AGGREGATE FULL SEASON -------------------
...
st.caption("Using game logs ONLY from 2025–26. No fallbacks.")
...
# ********************************************************************
# ✅ 3) BUILD PLAYER PROJECTIONS (cached + fast)
# ********************************************************************
st.markdown("### 3) Build Player Projections")

if st.button("📥 Build Projections"):

    props_preview = fetch_event_props(api_key, event_id, region, markets)
    player_names = set()

    # Extract players from props
    ...
    for pn in sorted(player_names):
        ...
        # ✅ cached team lookup
        team_name = get_team_name_cached(pid)
        if not team_name:
            continue

        # ✅ cached gamelog → FAST
        df = cached_gamelog(pid, SEASON)
...
# ✅ 4) RUN SIMULATION
# ********************************************************************
st.markdown("### 4) Run 10,000 Simulations")
