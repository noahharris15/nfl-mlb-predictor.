    ...
    for bk in props.get("bookmakers", []):
        for m in bk.get("markets", []):
            ...
            for o in m.get("outcomes", []):
                ...
                if mkey not in VALID_MARKETS: continue
                ...
                # ✅ RUN SIMULATION AND GET MODEL PROJECTION
                p_over, draws = t_over_prob(float(mu), float(sd), float(line), SIM_TRIALS)
                projection = float(np.median(draws))  # ✅ <— what the sim expects them to get
                ...
                rows.append({
                    ...
                    "line": line,
                    "Avg (raw)": round(mu,2),
                    "Model Projection": round(projection,2),   # ✅ ADDED
...
if full_slate:
    ...
    if not api_key:
        st.warning("Enter an Odds API key first.")
        st.stop()

    # Fetch entire slate (same lookahead)
    all_events = list_events(api_key, lookahead, region)
    if not all_events:
        st.error("No games returned for full slate.")
        st.stop()
    ...
    for ev in all_events:
        ...
        # Build projections for those players
        rows = []
        for pn in sorted(player_names):
            pid = find_player_id_by_name(pn)
            if not pid:
        ...
        for bk in props.get("bookmakers", []):
            for m in bk.get("markets", []):
                ...
                for o in m.get("outcomes", []):
                    ...
                    # Determine mu, sd just like original mode
                    def grab(col):
                        return row[f"mu_{col}"], row[f"sd_{col}"]
                    ...
                    edge = proj_val - line   # ✅ YOUR OPTION B
    ...
    if not master_rows:
        st.error("No props returned across the full slate.")
        st.stop()
